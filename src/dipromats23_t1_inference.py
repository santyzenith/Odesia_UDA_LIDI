...
dipromatst1_dataset = data_utils.prepare_dipromats_t1_test_dataset()

BASE_MODEL_ID = "meta-llama/Llama-3.1-8B-Instruct"
ADAPTER_ID = "santyzenith/Adapter-Llama-3.1-8B-odesia"
OUT_RUN_ID = ADAPTER_ID.split("/")[-1]
OUT_FILENAME = "DIPROMATS_2023_T1_es"
...
if __name__ == "__main__":
    dipromatst1_dataset['train'] = dipromatst1_dataset['train'].map(dipromatst1_inference,
                                                                    remove_columns=["text", "formatted_data"],
                                                                    batched=False
                                                                   )
    
    json_list = [dict(sample) for sample in dipromatst1_dataset['train']]
