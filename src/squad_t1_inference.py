...
squadt1_dataset = data_utils.prepare_squad_t1_test_dataset()
...
OUT_FILENAME = "SQAC_SQUAD_2024_es"
OUT_BASE_DIR = Path(__file__).resolve().parent.parent / "results" / "sqac_squad_2024"
...
if __name__ == "__main__":
    squadt1_dataset['train'] = squadt1_dataset['train'].map(squadt1_inference,
                                                            remove_columns=["formatted_data", "title", "context", "question"],
                                                            batched=False
                                                           )
    
    json_list = [dict(sample) for sample in squadt1_dataset['train']]
